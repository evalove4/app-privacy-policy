...
# 1. 페이지 설정
st.set_page_config(page_title="광천 Hills 주차위치 표시기 다운로드", page_icon="🏢", layout="centered")

# 2. 헤더 및 앱 소개
st.title("🏢 명품 광천 힐스APT 입주민 전용 앱")
st.info("이 앱은 우리 아파트 입주민들의 주차장 위치 확인을 위해 제작되었습니다.")
...
if os.path.exists(apk_file_path):
    ...
    3. **"보안상의 이유로..."** 라는 경고가 뜨면 **[설정]**을 누르고 **[이 소스 허용]**을 켜주세요.
...
해당 자료는 외부로 전송되거나 공유되지 않습니다. 해당 정보는 스마트폰 내부 SharedPreferences 에만 저장되며, 앱 삭제 시 함께 삭제됩니다. 또한 다른 앱에서 접근할 수 없도록 보호됩니다.

**3. App에서 요구하는 권한 **
사용자가 앱을 이용하는 동안 위치 정보나 기타 개인 식별 정보를 저장하거나 추적하지 않습니다. (블루투스 신호 수신을 위해 위치 장치(GPS)의 권한을 요청할 수 있으나, 실제 위치 정보는 수집하지 않습니다.)
